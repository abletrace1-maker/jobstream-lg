...
def map_to_job_processor(state: ParentGraphState) -> List[Send]:
    ...
    for job in scraped_jobs:
        ...
        # We send to the node named "process_job" (or similar, as defined in US-003, it says "use a stub node for 'process_job'")
        sends.append(Send("process_job", child_state))
...
def dummy_process_job(state: ChildGraphState) -> Dict[str, Any]:
    """
    Stub node for the child subgraph execution.
    """
    # This just returns an empty update or marks status
    return {}
...
builder.add_node("process_job", dummy_process_job)

# Add edges
builder.add_edge(START, "load_config_and_resume")
builder.add_edge("load_config_and_resume", "job_ingestion")
builder.add_conditional_edges("job_ingestion", map_to_job_processor, ["process_job"])
